package pwdhelper

import (
	"encoding/hex"
	"strings"
)

var crc8_tab = []byte{
	,
}

// 参数模型: CRC-8	x8 + x2 + x + 1
func Crc8(buf []byte) byte {
	var crc byte
	crc = 0
	var index = 0
	for ; index < len(buf); index++ {
		tempByte := buf[index]
		crc = crc8_tab[(crc^tempByte)&0xff] & 0xff
	}
	return crc
}

// 将16进制字符串CRC8后转换成16进制字符串
func Crc8ToString(str string) string {
	// 16进制字符串转换成[]byte{}
	b, _ := hex.DecodeString(str)
	// crc8后得到16进制byte
	data := Crc8(b)
	// byte转换成16进制字符串
	h := hex.EncodeToString([]byte{data})
	return strings.ToUpper(h)
}

// crc8校验, 参数模型: CRC-8/MAXIM	x8 + x5 + x4 + 1
func Crc8Check(str string) string {
	// 16进制字符串转换成[]byte{}
	data, _ := hex.DecodeString(str)

	var crc8 uint8 = 0
	l := len(data)
	for i := 0; i < l; i++ {
		crc8 ^= data[i]
		crc8 &= 0xff
		for j := 0; j < 8; j++ {
			if crc8&0x01 > 0 {
				crc8 = (crc8 >> 1) ^ 0x8C
				crc8 &= 0xff
			} else {
				crc8 >>= 1
			}
		}
		crc8 &= 0xff
	}
	crc := hex.EncodeToString([]byte{crc8})
	return crc
}
